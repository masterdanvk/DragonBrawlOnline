...
Where both turfs and objects are coded for use on the map.
destructible is a variable that if set to 1, will attempt to call Destroy_Landscape() on that object or
turf. The behavior of destruction can be customized in that proc, but turfs generally turn to brown dirt.
Trees are mapped as just the stump, the overlaying foliage is added as part of its initialization in New().
		...
		Destroy_Landscape()
			if(destructible)
				...
				spawn(5)
					...
					src.density=1
		Rock1
			icon='Icons/Tileset/props2x2.dmi'
			icon_state="rock"
			bound_width=64
			bound_height=64
		Rock2
			...
			bound_height=64
		Rock3
			icon='Icons/Tileset/props1x2.dmi'
			icon_state="rock"
			bound_width=32
			bound_height=64
		Rock4
